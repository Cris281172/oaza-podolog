<?php

return [
    'bezplatna-konsultacja-z-ulotka' => [
        'slug' => 'bezplatna-konsultacja-z-ulotka',

        'seo' => [
            'title' => 'Bezpłatna konsultacja podologiczna Kielce | OAZA',
            'description' => 'Bezpłatna konsultacja podologiczna w Kielcach z ulotką informacyjną. Wstępna ocena stanu stóp i dobór dalszej terapii.',
        ],

        'hero' => [
            'title' => 'Bezpłatna konsultacja',
            'titleSecond' => 'z ulotką',
            'text' => 'Wstępna konsultacja podologiczna wraz z materiałami informacyjnymi. Oceniamy stan stóp i wskazujemy możliwe kierunki dalszego leczenia.',
            'price_from' => '0 zł',
        ],

        'symptoms' => [
            'Pierwsze objawy problemów ze stopami',
            'Chęć profilaktycznej kontroli',
            'Potrzeba informacji o zabiegach podologicznych',
        ],

        'treatment' => [
            'title' => 'Na czym polega konsultacja?',
            'paragraphs' => [
                'Podczas wizyty przeprowadzany jest krótki wywiad oraz ocena stanu stóp.',
                'Pacjent otrzymuje ulotkę informacyjną dotyczącą najczęstszych problemów podologicznych.',
                'W razie potrzeby proponujemy dalszą diagnostykę lub zabieg.',
            ],
        ],

        'steps' => [
            [
                'title' => 'Rozmowa',
                'desc' => 'Krótki wywiad i poznanie problemu.',
            ],
            [
                'title' => 'Ocena',
                'desc' => 'Wstępne sprawdzenie stanu stóp.',
            ],
            [
                'title' => 'Rekomendacja',
                'desc' => 'Dobór dalszych kroków terapii.',
            ],
        ],

        'category' => 'konsultacje',

        'tags' => [
            'konsultacja',
            'profilaktyka',
            'pierwsza-wizyta',
        ],

    ],
    'bezplatna-konsultacja-w-ramach-zabiegu' => [
        'slug' => 'bezplatna-konsultacja-w-ramach-zabiegu',

        'seo' => [
            'title' => 'Konsultacja podologiczna w trakcie zabiegu | OAZA Kielce',
            'description' => 'Bezpłatna konsultacja podologiczna w ramach wykonywanego zabiegu w Kielcach. Ocena stanu stóp i plan terapii.',
        ],

        'hero' => [
            'title' => 'Konsultacja',
            'titleSecond' => 'w ramach zabiegu',
            'text' => 'Konsultacja podologiczna wykonywana podczas zabiegu. Obejmuje diagnostykę oraz dobór odpowiedniej terapii bez dodatkowych kosztów.',
            'price_from' => '0 zł',
        ],

        'symptoms' => [
            'Problem wymagający zabiegu podologicznego',
            'Potrzeba diagnozy przed terapią',
            'Pierwsza wizyta w gabinecie',
        ],

        'treatment' => [
            'title' => 'Jak wygląda konsultacja?',
            'paragraphs' => [
                'Konsultacja odbywa się bezpośrednio przed lub w trakcie zabiegu.',
                'Specjalista ocenia stan stóp i dobiera odpowiednią metodę terapii.',
                'Dzięki temu leczenie jest precyzyjnie dopasowane do problemu.',
            ],
        ],

        'steps' => [
            [
                'title' => 'Diagnoza',
                'desc' => 'Ocena problemu przed zabiegiem.',
            ],
            [
                'title' => 'Zabieg',
                'desc' => 'Wykonanie odpowiedniej terapii.',
            ],
            [
                'title' => 'Zalecenia',
                'desc' => 'Wskazówki po zabiegu.',
            ],
        ],

        'category' => 'konsultacje',

        'tags' => [
            'konsultacja',
            'zabieg',
            'diagnostyka',
            'pierwsza-wizyta',
        ],

    ],
    'konsultacja-podologiczna-w-gabinecie' => [
        'slug' => 'konsultacja-podologiczna-w-gabinecie',

        'seo' => [
            'title' => 'Konsultacja podologiczna Kielce 100 zł | OAZA',
            'description' => 'Profesjonalna konsultacja podologiczna w Kielcach. Ocena stanu stóp, plan terapii i indywidualne zalecenia.',
        ],

        'hero' => [
            'title' => 'Konsultacja podologiczna',
            'titleSecond' => 'w gabinecie',
            'text' => 'Szczegółowa konsultacja podologiczna obejmująca diagnostykę, ocenę stanu stóp oraz przygotowanie indywidualnego planu terapii.',
            'price_from' => '100 zł',
        ],

        'symptoms' => [
            'Ból lub dyskomfort stóp',
            'Nawracające problemy skórne',
            'Wrastające paznokcie',
            'Podejrzenie zmian chorobowych',
        ],

        'treatment' => [
            'title' => 'Co obejmuje konsultacja?',
            'paragraphs' => [
                'Szczegółowy wywiad medyczny oraz ocena stanu stóp.',
                'Ocena problemów takich jak wrastające paznokcie, odciski czy infekcje.',
                'Indywidualne zalecenia i plan terapii.',
            ],
        ],

        'steps' => [
            [
                'title' => 'Wywiad',
                'desc' => 'Zebranie informacji o problemie.',
            ],
            [
                'title' => 'Badanie',
                'desc' => 'Ocena stanu stóp i paznokci.',
            ],
            [
                'title' => 'Plan terapii',
                'desc' => 'Dobór terapii i zaleceń.',
            ],
        ],

        'category' => 'konsultacje',

        'tags' => [
            'konsultacja',
            'diagnostyka',
            'ból-stóp',
            'wrastające-paznokcie',
            'zmiany-skórne',
        ],

    ],
    'pedicure-dla-zdrowych-stop' => [
        'slug' => 'pedicure-dla-zdrowych-stop',

        'seo' => [
            'title' => 'Pedicure podologiczny Kielce | Zdrowe stopy | Podolog OAZA',
            'description' => 'Pedicure podologiczny dla zdrowych stóp w Kielcach. Opracowanie paznokci, wałów okołopaznokciowych, podeszwy oraz profesjonalna pielęgnacja.',
        ],

        'hero' => [
            'title' => 'Pedicure',
            'titleSecond' => 'podologiczny',
            'text' => 'Zabieg dla zdrowych stóp i paznokci obejmujący konsultację, skrócenie i opracowanie paznokci, opracowanie wałów, wygładzenie podeszwy oraz zastosowanie preparatów pielęgnacyjnych.',
            'price_from' => '170 zł',
        ],

        'symptoms' => [
            'Nadmierne rogowacenie skóry stóp',
            'Suchość i szorstkość podeszwy',
            'Dyskomfort związany z nieprawidłowym skracaniem paznokci',
            'Potrzeba regularnej profilaktycznej pielęgnacji',
        ],

        'treatment' => [
            'title' => 'Na czym polega zabieg?',
            'paragraphs' => [
                'Zabieg obejmuje skrócenie i opracowanie paznokci, oczyszczenie wałów okołopaznokciowych oraz wygładzenie podeszwy stóp.',
                'W trakcie wizyty stosowane są preparaty pielęgnacyjne wspierające regenerację skóry i poprawę kondycji stóp.',
                'Celem zabiegu jest profilaktyka problemów podologicznych, poprawa komfortu oraz utrzymanie stóp w dobrej kondycji.',
            ],
        ],

        'steps' => [
            [
                'title' => 'Konsultacja',
                'desc' => 'Ocena stanu stóp oraz potrzeb pielęgnacyjnych.',
            ],
            [
                'title' => 'Zabieg',
                'desc' => 'Opracowanie paznokci, wałów i podeszwy stóp.',
            ],
            [
                'title' => 'Pielęgnacja',
                'desc' => 'Dobór preparatów i zalecenia do dalszej profilaktyki.',
            ],
        ],

        'category' => 'profilaktyka',

        'tags' => [
            'pedicure podologiczny',
            'zdrowe stopy',
            'pielęgnacja stóp',
            'opracowanie paznokci',
        ],

    ],
    'stopa-cukrzycowa-i-luszczycowa' => [
        'slug' => 'stopa-cukrzycowa-i-luszczycowa',

        'seo' => [
            'title' => 'Stopa cukrzycowa i łuszczycowa Kielce | Podolog OAZA',
            'description' => 'Specjalistyczne opracowanie stopy cukrzycowej i łuszczycowej w Kielcach. Profilaktyka powikłań, pielęgnacja i terapia podologiczna.',
        ],

        'hero' => [
            'title' => 'Stopa cukrzycowa',
            'titleSecond' => 'i łuszczycowa',
            'text' => 'Specjalistyczny zabieg przeznaczony dla osób wymagających bezpiecznego opracowania podeszwy stopy zmienionej chorobowo, w tym stopy cukrzycowej i łuszczycowej.',
            'price_from' => '150 zł',
        ],

        'symptoms' => [
            'Suchość i pękanie skóry stóp',
            'Nadmierne rogowacenia i zgrubienia',
            'Zmiany łuszczycowe na stopach',
            'Problemy związane ze stopą cukrzycową',
            'Potrzeba profilaktyki powikłań',
        ],

        'treatment' => [
            'title' => 'Na czym polega zabieg?',
            'paragraphs' => [
                'Zabieg obejmuje specjalistyczne opracowanie podeszwy stopy oraz usunięcie problematycznych zrogowaceń.',
                'W przypadku stopy cukrzycowej szczególny nacisk kładziemy na profilaktykę powikłań i bezpieczeństwo terapii.',
                'Dobierane są również zalecenia pielęgnacyjne wspierające ochronę skóry oraz dalszą profilaktykę.',
            ],
        ],

        'steps' => [
            [
                'title' => 'Ocena',
                'desc' => 'Analiza stanu skóry, paznokci oraz zakresu problemu.',
            ],
            [
                'title' => 'Zabieg',
                'desc' => 'Specjalistyczne opracowanie zmian i dobranie terapii.',
            ],
            [
                'title' => 'Zalecenia',
                'desc' => 'Wskazówki dotyczące pielęgnacji oraz profilaktyki domowej.',
            ],
        ],

        'category' => 'stopy-zmienione-chorobowo',

        'tags' => [
            'stopa cukrzycowa',
            'stopa łuszczycowa',
            'łuszczyca stóp',
            'podolog cukrzyca',
            'opracowanie stopy',
        ],

    ],
    'terapia-pekajacych-piet' => [
        'slug' => 'terapia-pekajacych-piet',

        'seo' => [
            'title' => 'Terapia pękających pięt Kielce | Podolog OAZA',
            'description' => 'Leczenie pękających pięt w Kielcach. Opracowanie rozpadlin, usuwanie zrogowaceń i profesjonalna terapia podologiczna.',
        ],

        'hero' => [
            'title' => 'Terapia',
            'titleSecond' => 'pękających pięt',
            'text' => 'Specjalistyczny zabieg podologiczny przeznaczony dla osób z bolesnymi rozpadlinami, pęknięciami i nadmiernym rogowaceniem skóry pięt.',
            'price_from' => '180 zł',
        ],

        'symptoms' => [
            'Pękająca i przesuszona skóra pięt',
            'Bolesne rozpadliny',
            'Nadmierne zrogowacenia',
            'Dyskomfort podczas chodzenia',
            'Nawracający problem pękających pięt',
        ],

        'treatment' => [
            'title' => 'Na czym polega zabieg?',
            'paragraphs' => [
                'Zabieg obejmuje opracowanie pęknięć i rozpadlin oraz usunięcie nadmiernych zrogowaceń.',
                'W trakcie terapii dobierane są preparaty wspierające regenerację skóry oraz odbudowę bariery ochronnej.',
                'Celem zabiegu jest zmniejszenie bólu, poprawa komfortu chodzenia oraz ograniczenie nawrotów problemu.',
            ],
        ],

        'steps' => [
            [
                'title' => 'Ocena',
                'desc' => 'Analiza stanu skóry, paznokci oraz zakresu problemu.',
            ],
            [
                'title' => 'Zabieg',
                'desc' => 'Specjalistyczne opracowanie zmian i dobranie terapii.',
            ],
            [
                'title' => 'Zalecenia',
                'desc' => 'Wskazówki dotyczące pielęgnacji oraz profilaktyki domowej.',
            ],
        ],

        'category' => 'stopy-zmienione-chorobowo',

        'tags' => [
            'pękające pięty',
            'rozpadliny pięt',
            'leczenie pięt',
            'zrogowacenia pięt',
            'podolog pięty',
        ],

    ],
    'terapia-brodawki-wirusowej' => [
        'slug' => 'terapia-brodawki-wirusowej',

        'seo' => [
            'title' => 'Usuwanie brodawek wirusowych stóp Kielce | Podolog OAZA',
            'description' => 'Terapia brodawek wirusowych, czyli kurzajek, na stopach w Kielcach. Specjalistyczne opracowanie zmian i indywidualny plan terapii.',
        ],

        'hero' => [
            'title' => 'Terapia brodawki',
            'titleSecond' => 'wirusowej',
            'text' => 'Specjalistyczna terapia podologiczna mająca na celu opracowanie brodawek wirusowych, zahamowanie ich rozwoju oraz zapobieganie nawrotom.',
            'price_from' => '150 zł',
        ],

        'symptoms' => [
            'Zgrubienia na skórze stóp przypominające kurzajki',
            'Ból podczas chodzenia lub ucisku',
            'Punktowe, ciemne kropki w obrębie zmiany',
            'Rozprzestrzeniające się zmiany skórne',
            'Nawracające brodawki wirusowe',
        ],

        'treatment' => [
            'title' => 'Na czym polega terapia?',
            'paragraphs' => [
                'Terapia rozpoczyna się od dokładnej oceny zmiany oraz potwierdzenia jej charakteru wirusowego.',
                'Zabieg polega na miejscowym opracowaniu brodawki oraz zastosowaniu odpowiednio dobranej metody terapeutycznej.',
                'Pierwsza brodawka wyceniana jest od 150 zł, a kolejna od 30 zł.',
            ],
        ],

        'steps' => [
            [
                'title' => 'Ocena',
                'desc' => 'Analiza stanu skóry, paznokci oraz zakresu problemu.',
            ],
            [
                'title' => 'Zabieg',
                'desc' => 'Specjalistyczne opracowanie zmian i dobranie terapii.',
            ],
            [
                'title' => 'Zalecenia',
                'desc' => 'Wskazówki dotyczące pielęgnacji oraz profilaktyki domowej.',
            ],
        ],

        'category' => 'stopy-zmienione-chorobowo',

        'tags' => [
            'brodawka wirusowa',
            'kurzajki stopy',
            'usuwanie kurzajek',
            'leczenie brodawek',
            'podolog kurzajki',
        ],

        'short_desc' => 'Ocena zmian, miejscowe opracowanie brodawek wirusowych oraz wsparcie profilaktyki domowej.',
        'read_more_text' => 'Czytaj więcej →',

        'featured_card' => [
            'label' => 'Zabieg podologiczny 01',
            'title' => 'Terapia brodawki',
            'short_desc' => 'Ocena zmian, miejscowe opracowanie brodawek wirusowych oraz wsparcie profilaktyki domowej.',
            'read_more_text' => 'Czytaj więcej →',
        ],

        'label' => 'Zabieg podologiczny 01',

    ],
    'usuwanie-modzeli-i-odciskow' => [
        'slug' => 'usuwanie-modzeli-i-odciskow',

        'seo' => [
            'title' => 'Usuwanie modzeli i odcisków Kielce | Podolog OAZA',
            'description' => 'Profesjonalne usuwanie modzeli, odcisków i nagniotków w Kielcach. Skuteczna redukcja bólu, opracowanie zmian i profilaktyka nawrotów.',
        ],

        'hero' => [
            'title' => 'Usuwanie modzeli',
            'titleSecond' => 'i odcisków',
            'text' => 'Specjalistyczny zabieg podologiczny polegający na bezpiecznym usunięciu modzeli, odcisków i nagniotków oraz eliminacji przyczyn ich powstawania.',
            'price_from' => '100 zł',
        ],

        'symptoms' => [
            'Bolesne zgrubienia na podeszwie stóp',
            'Punktowy ból podczas chodzenia lub ucisku',
            'Twarde odciski z widocznym rdzeniem',
            'Nadmierne rogowacenia skóry',
            'Dyskomfort podczas noszenia obuwia',
        ],

        'treatment' => [
            'title' => 'Na czym polega zabieg?',
            'paragraphs' => [
                'Zabieg rozpoczyna się od dokładnej oceny zmian oraz określenia przyczyny ich powstawania.',
                'Podolog usuwa modzele, odciski lub nagniotki przy użyciu specjalistycznych narzędzi.',
                'Pierwsza zmiana wyceniana jest od 100 zł, a kolejna od 30 zł.',
            ],
        ],

        'steps' => [
            [
                'title' => 'Ocena',
                'desc' => 'Analiza stanu skóry, paznokci oraz zakresu problemu.',
            ],
            [
                'title' => 'Zabieg',
                'desc' => 'Specjalistyczne opracowanie zmian i dobranie terapii.',
            ],
            [
                'title' => 'Zalecenia',
                'desc' => 'Wskazówki dotyczące pielęgnacji oraz profilaktyki domowej.',
            ],
        ],

        'category' => 'stopy-zmienione-chorobowo',

        'tags' => [
            'modzele',
            'odciski',
            'nagniotki',
            'usuwanie odcisków',
            'leczenie modzeli',
        ],

    ],
    'obciecie-paznokci-zdrowych' => [
        'slug' => 'obciecie-paznokci-zdrowych',

        'seo' => [
            'title' => 'Obcięcie paznokci zdrowych Kielce | Podolog OAZA',
            'description' => 'Profesjonalne obcięcie zdrowych paznokci u stóp w Kielcach. Bezpieczna pielęgnacja w gabinecie podologicznym.',
        ],

        'hero' => [
            'title' => 'Obcięcie paznokci',
            'titleSecond' => 'zdrowych',
            'text' => 'Profesjonalny zabieg podologiczny polegający na prawidłowym skróceniu zdrowych paznokci u stóp z zachowaniem pełnego bezpieczeństwa.',
            'price_from' => '100 zł',
        ],

        'symptoms' => [
            'Trudność w samodzielnym obcinaniu paznokci',
            'Nieprawidłowy kształt paznokci wymagający korekty',
            'Profilaktyczna pielęgnacja stóp',
            'Potrzeba zachowania prawidłowej długości paznokci',
            'Chęć profesjonalnej higieny stóp',
        ],

        'treatment' => [
            'title' => 'Na czym polega zabieg?',
            'paragraphs' => [
                'Zabieg obejmuje skrócenie paznokci zgodnie z ich naturalnym kształtem.',
                'Podolog usuwa ewentualne nierówności i dba o bezpieczne opracowanie płytki.',
                'Na zakończenie pacjent otrzymuje zalecenia dotyczące prawidłowej pielęgnacji paznokci w domu.',
            ],
        ],

        'steps' => [
            [
                'title' => 'Ocena',
                'desc' => 'Analiza stanu skóry, paznokci oraz zakresu problemu.',
            ],
            [
                'title' => 'Zabieg',
                'desc' => 'Specjalistyczne opracowanie zmian i dobranie terapii.',
            ],
            [
                'title' => 'Zalecenia',
                'desc' => 'Wskazówki dotyczące pielęgnacji oraz profilaktyki domowej.',
            ],
        ],

        'category' => 'profilaktyka',

        'tags' => [
            'obcinanie paznokci',
            'paznokcie stóp',
            'pielęgnacja stóp',
            'podolog paznokcie',
        ],

    ],
    'opracowanie-paznokci-i-walow' => [
        'slug' => 'opracowanie-paznokci-i-walow',

        'seo' => [
            'title' => 'Opracowanie paznokci i wałów okołopaznokciowych Kielce | Podolog OAZA',
            'description' => 'Profesjonalne obcięcie i opracowanie zdrowych paznokci oraz wałów okołopaznokciowych w Kielcach.',
        ],

        'hero' => [
            'title' => 'Opracowanie paznokci',
            'titleSecond' => 'i wałów',
            'text' => 'Specjalistyczny zabieg obejmujący obcięcie i opracowanie zdrowych paznokci oraz dokładne oczyszczenie wałów okołopaznokciowych.',
            'price_from' => '150 zł',
        ],

        'symptoms' => [
            'Nadmierne narastanie skórek wokół paznokci',
            'Zalegający naskórek w wałach okołopaznokciowych',
            'Dyskomfort w obrębie paznokci',
            'Problemy z prawidłowym wzrostem paznokci',
            'Potrzeba regularnej pielęgnacji podologicznej',
        ],

        'treatment' => [
            'title' => 'Na czym polega zabieg?',
            'paragraphs' => [
                'Zabieg obejmuje dokładne oczyszczenie wałów okołopaznokciowych oraz usunięcie nadmiaru zrogowaciałego naskórka.',
                'Paznokcie są skracane i opracowywane w sposób bezpieczny, zgodny z ich naturalną budową.',
                'Na zakończenie stosowane są preparaty pielęgnacyjne oraz przekazywane zalecenia dotyczące dalszej profilaktyki.',
            ],
        ],

        'steps' => [
            [
                'title' => 'Ocena',
                'desc' => 'Analiza stanu skóry, paznokci oraz zakresu problemu.',
            ],
            [
                'title' => 'Zabieg',
                'desc' => 'Specjalistyczne opracowanie zmian i dobranie terapii.',
            ],
            [
                'title' => 'Zalecenia',
                'desc' => 'Wskazówki dotyczące pielęgnacji oraz profilaktyki domowej.',
            ],
        ],

        'category' => 'profilaktyka',

        'tags' => [
            'wały okołopaznokciowe',
            'opracowanie paznokci',
            'pielęgnacja paznokci',
            'podolog paznokcie',
        ],

    ],
    'paznokcie-dystroficzne' => [
        'slug' => 'paznokcie-dystroficzne',

        'seo' => [
            'title' => 'Leczenie paznokci dystroficznych Kielce | Podolog OAZA',
            'description' => 'Specjalistyczne opracowanie paznokci zmienionych chorobowo w Kielcach. Terapia, regeneracja i poprawa kondycji paznokci.',
        ],

        'hero' => [
            'title' => 'Paznokcie',
            'titleSecond' => 'dystroficzne',
            'text' => 'Specjalistyczny zabieg ukierunkowany na opracowanie paznokci zmienionych chorobowo, dystroficznych, zgrubiałych lub zniekształconych.',
            'price_from' => '200 zł',
        ],

        'symptoms' => [
            'Paznokcie zniekształcone i pogrubione',
            'Zmiana koloru płytki paznokciowej',
            'Kruchość i łamliwość paznokci',
            'Nierówna powierzchnia paznokcia',
            'Nawracające problemy z płytką paznokciową',
        ],

        'treatment' => [
            'title' => 'Na czym polega zabieg?',
            'paragraphs' => [
                'Zabieg polega na dokładnym opracowaniu zmienionej płytki paznokciowej oraz jej bezpiecznym skróceniu i wygładzeniu.',
                'W razie potrzeby usuwane są nadmiernie zrogowaciałe fragmenty oraz oczyszczane wały okołopaznokciowe.',
                'Dobierane są indywidualne preparaty wspierające regenerację paznokci oraz poprawę ich struktury.',
            ],
        ],

        'steps' => [
            [
                'title' => 'Diagnoza',
                'desc' => 'Ocena stanu i przyczyny zmian paznokci.',
            ],
            [
                'title' => 'Opracowanie',
                'desc' => 'Bezpieczne skrócenie i wygładzenie płytki.',
            ],
            [
                'title' => 'Regeneracja',
                'desc' => 'Zalecenia i terapia wspomagająca.',
            ],
        ],

        'category' => 'paznokcie-zmienione-chorobowo',

        'tags' => [
            'paznokcie dystroficzne',
            'zniekształcone paznokcie',
            'paznokcie zmienione chorobowo',
            'leczenie paznokci',
        ],

    ],
    'odbarczanie-krwiaka-podpaznokciowego' => [
        'slug' => 'odbarczanie-krwiaka-podpaznokciowego',

        'seo' => [
            'title' => 'Odbarczanie krwiaka podpaznokciowego Kielce | Podolog OAZA',
            'description' => 'Pomoc przy paznokciu po urazie w Kielcach. Odbarczanie krwiaka podpaznokciowego, oczyszczenie i zabezpieczenie płytki.',
        ],

        'hero' => [
            'title' => 'Odbarczanie krwiaka',
            'titleSecond' => 'podpaznokciowego',
            'text' => 'Zabieg przeznaczony dla paznokci po urazie, przy obecności krwiaka podpaznokciowego, bólu lub ucisku pod płytką paznokciową.',
            'price_from' => '150 zł',
        ],

        'symptoms' => [
            'Paznokieć po urazie lub uderzeniu',
            'Ból i ucisk pod paznokciem',
            'Ciemne zabarwienie płytki',
            'Tkliwość podczas chodzenia',
            'Ryzyko odklejenia paznokcia',
        ],

        'treatment' => [
            'title' => 'Na czym polega zabieg?',
            'paragraphs' => [
                'Podolog ocenia stan paznokcia po urazie oraz zakres krwiaka podpaznokciowego.',
                'W razie wskazań wykonywane jest bezpieczne odbarczenie i oczyszczenie miejsca urazu.',
                'Na zakończenie paznokieć zostaje zabezpieczony, a pacjent otrzymuje zalecenia dotyczące dalszej pielęgnacji.',
            ],
        ],

        'steps' => [
            [
                'title' => 'Ocena',
                'desc' => 'Sprawdzenie paznokcia po urazie.',
            ],
            [
                'title' => 'Odbarczenie',
                'desc' => 'Zmniejszenie ucisku i opracowanie miejsca urazu.',
            ],
            [
                'title' => 'Zabezpieczenie',
                'desc' => 'Opatrunek i zalecenia po zabiegu.',
            ],
        ],

        'category' => 'paznokcie-zmienione-chorobowo',

        'tags' => [
            'krwiak podpaznokciowy',
            'paznokieć po urazie',
            'uraz paznokcia',
            'ból paznokcia',
        ],

    ],
    'terapia-grzybicy-paznokci-i-stop' => [
        'slug' => 'terapia-grzybicy-paznokci-i-stop',

        'seo' => [
            'title' => 'Terapia grzybicy paznokci i stóp Kielce | Podolog OAZA',
            'description' => 'Diagnostyka i terapia grzybicy paznokci oraz stóp w Kielcach. Badanie mykologiczne, opracowanie zmian i preparaty specjalistyczne.',
        ],

        'hero' => [
            'title' => 'Terapia grzybicy',
            'titleSecond' => 'paznokci i stóp',
            'text' => 'Diagnostyka, badanie mykologiczne, opracowanie zmian grzybiczych oraz stosowanie odpowiednio dobranych preparatów.',
            'price_from' => '200 zł',
        ],

        'symptoms' => [
            'Zmiana koloru paznokcia',
            'Pogrubienie lub kruchość płytki',
            'Łuszczenie i świąd skóry stóp',
            'Onycholiza lub podejrzenie infekcji',
            'Zmiany typu pseudomonas, paznokcie szponowate lub po leczeniu onkologicznym',
        ],

        'treatment' => [
            'title' => 'Na czym polega terapia?',
            'paragraphs' => [
                'Terapia rozpoczyna się od diagnostyki oraz, w razie potrzeby, pobrania materiału do badania mykologicznego.',
                'Zabieg obejmuje opracowanie paznokci ze zmianami grzybiczymi, łuszczycowymi, szponowatymi, po leczeniu onkologicznym oraz zmian takich jak onycholiza czy pseudomonas.',
                'Na podstawie obrazu zmian i wyników badań dobierane są preparaty oraz zalecenia do dalszej terapii.',
            ],
        ],

        'steps' => [
            [
                'title' => 'Diagnostyka',
                'desc' => 'Ocena zmian i kwalifikacja do badania.',
            ],
            [
                'title' => 'Opracowanie',
                'desc' => 'Oczyszczenie i opracowanie zmienionych paznokci lub skóry.',
            ],
            [
                'title' => 'Terapia',
                'desc' => 'Preparaty specjalistyczne i profilaktyka domowa.',
            ],
        ],

        'category' => 'paznokcie-zmienione-chorobowo',

        'tags' => [
            'grzybica paznokci',
            'grzybica stóp',
            'onycholiza',
            'pseudomonas',
            'paznokcie szponowate',
        ],

        'short_desc' => 'Ocena problemu, badanie mykologiczne, opracowanie zmian grzybiczych i stosowanie preparatów.',
        'read_more_text' => 'Czytaj więcej →',

        'featured_card' => [
            'label' => 'Zabieg podologiczny 02',
            'title' => 'Terapia grzybicy paznokci i stóp',
            'short_desc' => 'Ocena problemu, badanie mykologiczne, opracowanie zmian grzybiczych i stosowanie preparatów.',
            'read_more_text' => 'Czytaj więcej →',
        ],

        'label' => 'Zabieg podologiczny 02',

    ],
    'podstawowy-zabieg-podologiczny' => [
        'slug' => 'podstawowy-zabieg-podologiczny',

        'seo' => [
            'title' => 'Podstawowy zabieg podologiczny Kielce | Podolog OAZA',
            'description' => 'Podstawowy zabieg podologiczny w Kielcach. Usunięcie mniejszych zrogowaceń, opracowanie paznokci i wałów oraz preparaty pielęgnacyjne.',
        ],

        'hero' => [
            'title' => 'Podstawowy zabieg',
            'titleSecond' => 'podologiczny',
            'text' => 'Kompleksowy zabieg obejmujący usunięcie mniejszych fizjologicznych zrogowaceń, opracowanie paznokci i wałów okołopaznokciowych oraz nałożenie preparatów.',
            'price_from' => '200 zł',
        ],

        'symptoms' => [
            'Potrzeba regularnej pielęgnacji stóp',
            'Mniejsze fizjologiczne zrogowacenia',
            'Problemy z prawidłowym skracaniem paznokci',
            'Dyskomfort związany ze stanem stóp',
            'Chęć profilaktyki podologicznej',
        ],

        'treatment' => [
            'title' => 'Na czym polega zabieg?',
            'paragraphs' => [
                'Zabieg obejmuje skrócenie i opracowanie paznokci oraz usunięcie mniejszych zrogowaceń skóry stóp.',
                'W trakcie wizyty wykonywane jest również oczyszczenie wałów okołopaznokciowych oraz podstawowa pielęgnacja skóry.',
                'Na zakończenie pacjent otrzymuje zalecenia dotyczące dalszej pielęgnacji i profilaktyki.',
            ],
        ],

        'steps' => [
            [
                'title' => 'Ocena',
                'desc' => 'Sprawdzenie stanu stóp i paznokci.',
            ],
            [
                'title' => 'Zabieg',
                'desc' => 'Opracowanie paznokci, wałów i skóry.',
            ],
            [
                'title' => 'Profilaktyka',
                'desc' => 'Preparaty i zalecenia pielęgnacyjne.',
            ],
        ],

        'category' => 'kompleksowe-zabiegi',

        'tags' => [
            'zabieg podologiczny',
            'podstawowy zabieg',
            'pielęgnacja stóp',
            'podolog kielce',
        ],

    ],
    'rozszerzony-zabieg-podologiczny' => [
        'slug' => 'rozszerzony-zabieg-podologiczny',

        'seo' => [
            'title' => 'Rozszerzony zabieg podologiczny Kielce | Podolog OAZA',
            'description' => 'Rozszerzony zabieg podologiczny w Kielcach. Usunięcie średnich zrogowaceń, opracowanie paznokci, wałów i pielęgnacja skóry.',
        ],

        'hero' => [
            'title' => 'Rozszerzony zabieg',
            'titleSecond' => 'podologiczny',
            'text' => 'Kompleksowy zabieg obejmujący usunięcie średnich fizjologicznych zrogowaceń, opracowanie paznokci i wałów okołopaznokciowych oraz nałożenie preparatów.',
            'price_from' => '300 zł',
        ],

        'symptoms' => [
            'Średnie fizjologiczne zrogowacenia',
            'Większy zakres pielęgnacji skóry stóp',
            'Problemy z paznokciami i skórą jednocześnie',
            'Nawracające dolegliwości stóp',
            'Potrzeba bardziej zaawansowanej pielęgnacji',
        ],

        'treatment' => [
            'title' => 'Na czym polega zabieg?',
            'paragraphs' => [
                'Zabieg obejmuje dokładne opracowanie paznokci, wałów okołopaznokciowych oraz skóry stóp z usunięciem średnich zrogowaceń.',
                'W razie potrzeby wykonywane jest także opracowanie miejsc wymagających większej interwencji podologicznej.',
                'Na zakończenie dobierane są indywidualne zalecenia pielęgnacyjne oraz profilaktyczne.',
            ],
        ],

        'steps' => [
            [
                'title' => 'Diagnoza',
                'desc' => 'Ocena stanu stóp i zakresu problemów.',
            ],
            [
                'title' => 'Zabieg',
                'desc' => 'Rozszerzone opracowanie paznokci, wałów i skóry.',
            ],
            [
                'title' => 'Zalecenia',
                'desc' => 'Indywidualna profilaktyka i pielęgnacja.',
            ],
        ],

        'category' => 'kompleksowe-zabiegi',

        'tags' => [
            'zabieg podologiczny',
            'rozszerzony zabieg',
            'pielęgnacja stóp',
            'podolog kielce',
        ],

    ],
    'pomiar-poziomu-cukru' => [
        'slug' => 'pomiar-poziomu-cukru',

        'seo' => [
            'title' => 'Pomiar poziomu cukru Kielce | Podolog OAZA',
            'description' => 'Szybkie i kontrolne oznaczenie poziomu glukozy we krwi, szczególnie istotne dla pacjentów z cukrzycą oraz przed zabiegami podologicznymi. Kielce, gabinet podologiczny OAZA.',
        ],

        'hero' => [
            'title' => 'Pomiar poziomu',
            'titleSecond' => 'cukru',
            'text' => 'Szybkie i kontrolne oznaczenie poziomu glukozy we krwi, szczególnie istotne dla pacjentów z cukrzycą oraz przed zabiegami podologicznymi.',
            'price_from' => '50 zł',
        ],

        'symptoms' => [
            'Podejrzenie infekcji grzybiczej lub bakteryjnej',
            'Zmiany koloru i struktury paznokci',
            'Nawracające stany zapalne',
            'Potrzeba potwierdzenia przyczyny zmian',
            'Dobór dalszej terapii na podstawie wyniku',
        ],

        'treatment' => [
            'title' => 'Na czym polega badanie?',
            'paragraphs' => [
                'Badanie rozpoczyna się od wywiadu oraz oceny zmian w obrębie skóry lub paznokci.',
                'Następnie pobierany jest odpowiedni materiał diagnostyczny, np. fragment paznokcia, zeskrobina lub wymaz.',
                'Wynik badania pozwala dobrać właściwy kierunek terapii podologicznej lub lekarskiej.',
            ],
        ],

        'steps' => [
            [
                'title' => 'Wywiad',
                'desc' => 'Ocena objawów i historii problemu.',
            ],
            [
                'title' => 'Pobranie',
                'desc' => 'Pobranie materiału do badania.',
            ],
            [
                'title' => 'Wynik',
                'desc' => 'Omówienie dalszych zaleceń po analizie.',
            ],
        ],

        'category' => 'diagnostyka',

        'tags' => [
            'pomiar cukru',
            'glukoza we krwi',
            'cukrzyca',
            'kontrola glikemii',
        ],

    ],
    'badanie-mykologiczne-bezposrednie' => [
        'slug' => 'badanie-mykologiczne-bezposrednie',

        'seo' => [
            'title' => 'Badanie mykologiczne bezpośrednie Kielce | Podolog OAZA',
            'description' => 'Badanie mykologiczne bezpośrednie wykonywane w celu wykrycia nici grzyba w materiale pobranym ze skóry lub paznokcia. Kielce, gabinet podologiczny OAZA.',
        ],

        'hero' => [
            'title' => 'Badanie mykologiczne',
            'titleSecond' => 'bezpośrednie',
            'text' => 'Badanie mykologiczne bezpośrednie wykonywane w celu wykrycia nici grzyba w materiale pobranym ze skóry lub paznokcia.',
            'price_from' => '300 zł',
        ],

        'symptoms' => [
            'Podejrzenie infekcji grzybiczej lub bakteryjnej',
            'Zmiany koloru i struktury paznokci',
            'Nawracające stany zapalne',
            'Potrzeba potwierdzenia przyczyny zmian',
            'Dobór dalszej terapii na podstawie wyniku',
        ],

        'treatment' => [
            'title' => 'Na czym polega badanie?',
            'paragraphs' => [
                'Badanie rozpoczyna się od wywiadu oraz oceny zmian w obrębie skóry lub paznokci.',
                'Następnie pobierany jest odpowiedni materiał diagnostyczny, np. fragment paznokcia, zeskrobina lub wymaz.',
                'Wynik badania pozwala dobrać właściwy kierunek terapii podologicznej lub lekarskiej.',
            ],
        ],

        'steps' => [
            [
                'title' => 'Wywiad',
                'desc' => 'Ocena objawów i historii problemu.',
            ],
            [
                'title' => 'Pobranie',
                'desc' => 'Pobranie materiału do badania.',
            ],
            [
                'title' => 'Wynik',
                'desc' => 'Omówienie dalszych zaleceń po analizie.',
            ],
        ],

        'category' => 'diagnostyka',

        'tags' => [
            'badanie mykologiczne',
            'nici grzyba',
            'grzybica paznokci',
            'diagnostyka',
        ],

    ],
    'badanie-mykologiczne-hodowla' => [
        'slug' => 'badanie-mykologiczne-hodowla',

        'seo' => [
            'title' => 'Badanie mykologiczne hodowla Kielce | Podolog OAZA',
            'description' => 'Badanie mykologiczne w formie hodowli, wykonywane w celu identyfikacji gatunku grzyba odpowiedzialnego za infekcję. Kielce, gabinet podologiczny OAZA.',
        ],

        'hero' => [
            'title' => 'Badanie mykologiczne',
            'titleSecond' => 'hodowla',
            'text' => 'Badanie mykologiczne w formie hodowli, wykonywane w celu identyfikacji gatunku grzyba odpowiedzialnego za infekcję.',
            'price_from' => '300 zł',
        ],

        'symptoms' => [
            'Podejrzenie infekcji grzybiczej lub bakteryjnej',
            'Zmiany koloru i struktury paznokci',
            'Nawracające stany zapalne',
            'Potrzeba potwierdzenia przyczyny zmian',
            'Dobór dalszej terapii na podstawie wyniku',
        ],

        'treatment' => [
            'title' => 'Na czym polega badanie?',
            'paragraphs' => [
                'Badanie rozpoczyna się od wywiadu oraz oceny zmian w obrębie skóry lub paznokci.',
                'Następnie pobierany jest odpowiedni materiał diagnostyczny, np. fragment paznokcia, zeskrobina lub wymaz.',
                'Wynik badania pozwala dobrać właściwy kierunek terapii podologicznej lub lekarskiej.',
            ],
        ],

        'steps' => [
            [
                'title' => 'Wywiad',
                'desc' => 'Ocena objawów i historii problemu.',
            ],
            [
                'title' => 'Pobranie',
                'desc' => 'Pobranie materiału do badania.',
            ],
            [
                'title' => 'Wynik',
                'desc' => 'Omówienie dalszych zaleceń po analizie.',
            ],
        ],

        'category' => 'diagnostyka',

        'tags' => [
            'hodowla grzybów',
            'badanie mykologiczne',
            'identyfikacja grzyba',
            'grzybica',
        ],

    ],
    'badanie-mykologiczne-kompleksowe' => [
        'slug' => 'badanie-mykologiczne-kompleksowe',

        'seo' => [
            'title' => 'Badanie mykologiczne kompleksowe Kielce | Podolog OAZA',
            'description' => 'Badanie obejmujące diagnostykę bezpośrednią oraz badanie mykologiczne wspierające dokładniejszą ocenę infekcji. Kielce, gabinet podologiczny OAZA.',
        ],

        'hero' => [
            'title' => 'Badanie mykologiczne',
            'titleSecond' => 'kompleksowe',
            'text' => 'Badanie obejmujące diagnostykę bezpośrednią oraz badanie mykologiczne wspierające dokładniejszą ocenę infekcji.',
            'price_from' => '350 zł',
        ],

        'symptoms' => [
            'Podejrzenie infekcji grzybiczej lub bakteryjnej',
            'Zmiany koloru i struktury paznokci',
            'Nawracające stany zapalne',
            'Potrzeba potwierdzenia przyczyny zmian',
            'Dobór dalszej terapii na podstawie wyniku',
        ],

        'treatment' => [
            'title' => 'Na czym polega badanie?',
            'paragraphs' => [
                'Badanie rozpoczyna się od wywiadu oraz oceny zmian w obrębie skóry lub paznokci.',
                'Następnie pobierany jest odpowiedni materiał diagnostyczny, np. fragment paznokcia, zeskrobina lub wymaz.',
                'Wynik badania pozwala dobrać właściwy kierunek terapii podologicznej lub lekarskiej.',
            ],
        ],

        'steps' => [
            [
                'title' => 'Wywiad',
                'desc' => 'Ocena objawów i historii problemu.',
            ],
            [
                'title' => 'Pobranie',
                'desc' => 'Pobranie materiału do badania.',
            ],
            [
                'title' => 'Wynik',
                'desc' => 'Omówienie dalszych zaleceń po analizie.',
            ],
        ],

        'category' => 'diagnostyka',

        'tags' => [
            'badanie mykologiczne kompleksowe',
            'grzybica paznokci',
            'diagnostyka grzybicy',
        ],

    ],
    'badanie-mikrobiologiczne-z-mykogramem' => [
        'slug' => 'badanie-mikrobiologiczne-z-mykogramem',

        'seo' => [
            'title' => 'Badanie mikrobiologiczne z mykogramem Kielce | Podolog OAZA',
            'description' => 'Badanie mikrobiologiczne z mykogramem, wykonywane m.in. przy stanie zapalnym, wysięku lub podejrzeniu infekcji mieszanej. Kielce, gabinet podologiczny OAZA.',
        ],

        'hero' => [
            'title' => 'Badanie mikrobiologiczne',
            'titleSecond' => 'z mykogramem',
            'text' => 'Badanie mikrobiologiczne z mykogramem, wykonywane m.in. przy stanie zapalnym, wysięku lub podejrzeniu infekcji mieszanej.',
            'price_from' => '350 zł',
        ],

        'symptoms' => [
            'Podejrzenie infekcji grzybiczej lub bakteryjnej',
            'Zmiany koloru i struktury paznokci',
            'Nawracające stany zapalne',
            'Potrzeba potwierdzenia przyczyny zmian',
            'Dobór dalszej terapii na podstawie wyniku',
        ],

        'treatment' => [
            'title' => 'Na czym polega badanie?',
            'paragraphs' => [
                'Badanie rozpoczyna się od wywiadu oraz oceny zmian w obrębie skóry lub paznokci.',
                'Następnie pobierany jest odpowiedni materiał diagnostyczny, np. fragment paznokcia, zeskrobina lub wymaz.',
                'Wynik badania pozwala dobrać właściwy kierunek terapii podologicznej lub lekarskiej.',
            ],
        ],

        'steps' => [
            [
                'title' => 'Wywiad',
                'desc' => 'Ocena objawów i historii problemu.',
            ],
            [
                'title' => 'Pobranie',
                'desc' => 'Pobranie materiału do badania.',
            ],
            [
                'title' => 'Wynik',
                'desc' => 'Omówienie dalszych zaleceń po analizie.',
            ],
        ],

        'category' => 'diagnostyka',

        'tags' => [
            'badanie mikrobiologiczne',
            'mykogram',
            'stan zapalny',
            'wymaz',
        ],

    ],
    'badanie-genetyczne-hla-cw6' => [
        'slug' => 'badanie-genetyczne-hla-cw6',

        'seo' => [
            'title' => 'Badanie genetyczne HLA-CW6 Kielce | Podolog OAZA',
            'description' => 'Badanie genetyczne w kierunku łuszczycy, obejmujące oznaczenie genu HLA-CW6. Kielce, gabinet podologiczny OAZA.',
        ],

        'hero' => [
            'title' => 'Badanie genetyczne',
            'titleSecond' => 'HLA-CW6',
            'text' => 'Badanie genetyczne w kierunku łuszczycy, obejmujące oznaczenie genu HLA-CW6.',
        ],

        'symptoms' => [
            'Podejrzenie infekcji grzybiczej lub bakteryjnej',
            'Zmiany koloru i struktury paznokci',
            'Nawracające stany zapalne',
            'Potrzeba potwierdzenia przyczyny zmian',
            'Dobór dalszej terapii na podstawie wyniku',
        ],

        'treatment' => [
            'title' => 'Na czym polega badanie?',
            'paragraphs' => [
                'Badanie rozpoczyna się od wywiadu oraz oceny zmian w obrębie skóry lub paznokci.',
                'Następnie pobierany jest odpowiedni materiał diagnostyczny, np. fragment paznokcia, zeskrobina lub wymaz.',
                'Wynik badania pozwala dobrać właściwy kierunek terapii podologicznej lub lekarskiej.',
            ],
        ],

        'steps' => [
            [
                'title' => 'Wywiad',
                'desc' => 'Ocena objawów i historii problemu.',
            ],
            [
                'title' => 'Pobranie',
                'desc' => 'Pobranie materiału do badania.',
            ],
            [
                'title' => 'Wynik',
                'desc' => 'Omówienie dalszych zaleceń po analizie.',
            ],
        ],

        'category' => 'diagnostyka',

        'tags' => [
            'badanie genetyczne',
            'HLA-CW6',
            'łuszczyca',
            'diagnostyka',
        ],

    ],
    'terapia-wrastajacych-paznokci' => [
        'slug' => 'terapia-wrastajacych-paznokci',

        'seo' => [
            'title' => 'Terapia wrastających paznokci Kielce | Podolog OAZA',
            'description' => 'Terapia wrastających paznokci w Kielcach. Opracowanie paznokcia i wałów, klamry ortonyksyjne, tamponada, taping i opatrunki.',
        ],

        'hero' => [
            'title' => 'Terapia wrastających',
            'titleSecond' => 'paznokci',
            'text' => 'Ocena problemu, opracowanie paznokcia i wałów okołopaznokciowych, założenie klamry, odciążeń, tamponady, tapingu lub opatrunku.',
            'price_from' => 'od 50 zł',
        ],

        'symptoms' => [
            'Wrastający lub wkręcający się paznokieć',
            'Ból przy ucisku lub chodzeniu',
            'Stan zapalny wałów okołopaznokciowych',
            'Zaczerwienienie i obrzęk przy paznokciu',
            'Nawracające problemy z paznokciami',
        ],

        'treatment' => [
            'title' => 'Na czym polega terapia?',
            'paragraphs' => [
                'Terapia rozpoczyna się od diagnostyki oraz oceny stopnia wrastania paznokcia.',
                'W zależności od problemu wykonywane jest opracowanie paznokcia i wałów okołopaznokciowych, założenie klamry, tamponady, tapingu lub opatrunku.',
                'Celem jest zmniejszenie bólu, odciążenie tkanek oraz wsparcie prawidłowego wzrostu płytki.',
            ],
        ],

        'steps' => [
            [
                'title' => 'Ocena problemu',
                'desc' => 'Ocena paznokcia i wałów okołopaznokciowych.',
            ],
            [
                'title' => 'Terapia',
                'desc' => 'Opracowanie, klamra, tamponada, taping lub opatrunek.',
            ],
            [
                'title' => 'Kontrola',
                'desc' => 'Monitorowanie efektów i dalsze zalecenia.',
            ],
        ],

        'category' => 'wrastajace-paznokcie',

        'tags' => [
            'wrastające paznokcie',
            'klamra ortonyksyjna',
            'tamponada',
            'taping',
            'opatrunek',
        ],

        'short_desc' => 'Ocena problemu, opracowanie paznokcia i wałów okołopaznokciowych, założenie klamry i odciążeń.',
        'read_more_text' => 'Czytaj więcej →',

        'featured_card' => [
            'label' => 'Zabieg podologiczny 03',
            'title' => 'Terapia wrastających paznokci',
            'short_desc' => 'Ocena problemu, opracowanie paznokcia i wałów okołopaznokciowych, założenie klamry i odciążeń.',
            'read_more_text' => 'Czytaj więcej →',
        ],

        'label' => 'Zabieg podologiczny 03',

    ],
    'klamry-ortonyksyjne' => [
        'slug' => 'klamry-ortonyksyjne',

        'seo' => [
            'title' => 'Klamry ortonyksyjne Kielce | Podolog OAZA',
            'description' => 'Specjalistyczna metoda korekcji wrastających i wkręcających się paznokci polegająca na zastosowaniu klamry ortonyksyjnej przywracającej prawidłowy tor wzrostu płytki. Gabinet podologiczny OAZA Kielce.',
        ],

        'hero' => [
            'title' => 'Klamry',
            'titleSecond' => 'ortonyksyjne',
            'text' => 'Specjalistyczna metoda korekcji wrastających i wkręcających się paznokci polegająca na zastosowaniu klamry ortonyksyjnej przywracającej prawidłowy tor wzrostu płytki.',
            'price_from' => '200 zł',
        ],

        'symptoms' => [
            'Wrastający lub bolesny paznokieć',
            'Stan zapalny wałów okołopaznokciowych',
            'Ból i ucisk podczas chodzenia',
            'Potrzeba zabezpieczenia miejsca po zabiegu',
            'Kontynuacja terapii podologicznej',
        ],

        'treatment' => [
            'title' => 'Na czym polega zabieg?',
            'paragraphs' => [
                'Podolog ocenia aktualny stan paznokcia, skóry i tkanek okołopaznokciowych.',
                'Następnie dobierana jest odpowiednia metoda zabezpieczenia, korekcji lub odciążenia.',
                'Na zakończenie pacjent otrzymuje zalecenia dotyczące dalszej pielęgnacji oraz ewentualnej kontroli.',
            ],
        ],

        'steps' => [
            [
                'title' => 'Ocena',
                'desc' => 'Analiza paznokcia i tkanek.',
            ],
            [
                'title' => 'Zabieg',
                'desc' => 'Wykonanie korekcji, zabezpieczenia lub odciążenia.',
            ],
            [
                'title' => 'Kontrola',
                'desc' => 'Zalecenia i dalsza opieka.',
            ],
        ],

        'category' => 'wrastajace-paznokcie',

        'tags' => [
            'klamry ortonyksyjne',
            'wrastające paznokcie',
            'wkręcające paznokcie',
        ],

    ],
    'przelozenie-klamry' => [
        'slug' => 'przelozenie-klamry',

        'seo' => [
            'title' => 'Przełożenie klamry Kielce | Podolog OAZA',
            'description' => 'Wizyta kontrolna w trakcie terapii wrastającego paznokcia, obejmująca przełożenie lub korektę działania klamry ortonyksyjnej. Gabinet podologiczny OAZA Kielce.',
        ],

        'hero' => [
            'title' => 'Przełożenie',
            'titleSecond' => 'klamry',
            'text' => 'Wizyta kontrolna w trakcie terapii wrastającego paznokcia, obejmująca przełożenie lub korektę działania klamry ortonyksyjnej.',
            'price_from' => '150 zł',
        ],

        'symptoms' => [
            'Wrastający lub bolesny paznokieć',
            'Stan zapalny wałów okołopaznokciowych',
            'Ból i ucisk podczas chodzenia',
            'Potrzeba zabezpieczenia miejsca po zabiegu',
            'Kontynuacja terapii podologicznej',
        ],

        'treatment' => [
            'title' => 'Na czym polega zabieg?',
            'paragraphs' => [
                'Podolog ocenia aktualny stan paznokcia, skóry i tkanek okołopaznokciowych.',
                'Następnie dobierana jest odpowiednia metoda zabezpieczenia, korekcji lub odciążenia.',
                'Na zakończenie pacjent otrzymuje zalecenia dotyczące dalszej pielęgnacji oraz ewentualnej kontroli.',
            ],
        ],

        'steps' => [
            [
                'title' => 'Ocena',
                'desc' => 'Analiza paznokcia i tkanek.',
            ],
            [
                'title' => 'Zabieg',
                'desc' => 'Wykonanie korekcji, zabezpieczenia lub odciążenia.',
            ],
            [
                'title' => 'Kontrola',
                'desc' => 'Zalecenia i dalsza opieka.',
            ],
        ],

        'category' => 'wrastajace-paznokcie',

        'tags' => [
            'przełożenie klamry',
            'kontrola klamry',
            'wrastające paznokcie',
        ],

    ],
    'zdjecie-klamry-ortonyksyjnej' => [
        'slug' => 'zdjecie-klamry-ortonyksyjnej',

        'seo' => [
            'title' => 'Zdjęcie klamry ortonyksyjnej Kielce | Podolog OAZA',
            'description' => 'Wizyta kontrolna obejmująca ocenę efektów terapii oraz bezpieczne zdjęcie klamry ortonyksyjnej z paznokcia. Gabinet podologiczny OAZA Kielce.',
        ],

        'hero' => [
            'title' => 'Zdjęcie klamry',
            'titleSecond' => 'ortonyksyjnej',
            'text' => 'Wizyta kontrolna obejmująca ocenę efektów terapii oraz bezpieczne zdjęcie klamry ortonyksyjnej z paznokcia.',
            'price_from' => '100 zł',
        ],

        'symptoms' => [
            'Wrastający lub bolesny paznokieć',
            'Stan zapalny wałów okołopaznokciowych',
            'Ból i ucisk podczas chodzenia',
            'Potrzeba zabezpieczenia miejsca po zabiegu',
            'Kontynuacja terapii podologicznej',
        ],

        'treatment' => [
            'title' => 'Na czym polega zabieg?',
            'paragraphs' => [
                'Podolog ocenia aktualny stan paznokcia, skóry i tkanek okołopaznokciowych.',
                'Następnie dobierana jest odpowiednia metoda zabezpieczenia, korekcji lub odciążenia.',
                'Na zakończenie pacjent otrzymuje zalecenia dotyczące dalszej pielęgnacji oraz ewentualnej kontroli.',
            ],
        ],

        'steps' => [
            [
                'title' => 'Ocena',
                'desc' => 'Analiza paznokcia i tkanek.',
            ],
            [
                'title' => 'Zabieg',
                'desc' => 'Wykonanie korekcji, zabezpieczenia lub odciążenia.',
            ],
            [
                'title' => 'Kontrola',
                'desc' => 'Zalecenia i dalsza opieka.',
            ],
        ],

        'category' => 'wrastajace-paznokcie',

        'tags' => [
            'zdjęcie klamry',
            'klamra ortonyksyjna',
            'kontrola',
        ],

    ],
    'tamponada' => [
        'slug' => 'tamponada',

        'seo' => [
            'title' => 'Tamponada podologiczna Kielce | Podolog OAZA',
            'description' => 'Delikatne zabezpieczenie wału okołopaznokciowego przy wrastającym paznokciu w celu zmniejszenia ucisku i bólu. Gabinet podologiczny OAZA Kielce.',
        ],

        'hero' => [
            'title' => 'Tamponada',
            'titleSecond' => 'podologiczna',
            'text' => 'Delikatne zabezpieczenie wału okołopaznokciowego przy wrastającym paznokciu w celu zmniejszenia ucisku i bólu.',
            'price_from' => '50 zł',
        ],

        'symptoms' => [
            'Wrastający lub bolesny paznokieć',
            'Stan zapalny wałów okołopaznokciowych',
            'Ból i ucisk podczas chodzenia',
            'Potrzeba zabezpieczenia miejsca po zabiegu',
            'Kontynuacja terapii podologicznej',
        ],

        'treatment' => [
            'title' => 'Na czym polega zabieg?',
            'paragraphs' => [
                'Podolog ocenia aktualny stan paznokcia, skóry i tkanek okołopaznokciowych.',
                'Następnie dobierana jest odpowiednia metoda zabezpieczenia, korekcji lub odciążenia.',
                'Na zakończenie pacjent otrzymuje zalecenia dotyczące dalszej pielęgnacji oraz ewentualnej kontroli.',
            ],
        ],

        'steps' => [
            [
                'title' => 'Ocena',
                'desc' => 'Analiza paznokcia i tkanek.',
            ],
            [
                'title' => 'Zabieg',
                'desc' => 'Wykonanie korekcji, zabezpieczenia lub odciążenia.',
            ],
            [
                'title' => 'Kontrola',
                'desc' => 'Zalecenia i dalsza opieka.',
            ],
        ],

        'category' => 'wrastajace-paznokcie',

        'tags' => [
            'tamponada',
            'wrastający paznokieć',
            'ból paznokcia',
        ],

    ],
    'taping-podologiczny' => [
        'slug' => 'taping-podologiczny',

        'seo' => [
            'title' => 'Taping podologiczny Kielce | Podolog OAZA',
            'description' => 'Odciążenie i wsparcie tkanek przy pomocy tapingu podologicznego, stosowane m.in. przy wrastających paznokciach i dolegliwościach bólowych. Gabinet podologiczny OAZA Kielce.',
        ],

        'hero' => [
            'title' => 'Taping',
            'titleSecond' => 'podologiczny',
            'text' => 'Odciążenie i wsparcie tkanek przy pomocy tapingu podologicznego, stosowane m.in. przy wrastających paznokciach i dolegliwościach bólowych.',
            'price_from' => '50 zł',
        ],

        'symptoms' => [
            'Wrastający lub bolesny paznokieć',
            'Stan zapalny wałów okołopaznokciowych',
            'Ból i ucisk podczas chodzenia',
            'Potrzeba zabezpieczenia miejsca po zabiegu',
            'Kontynuacja terapii podologicznej',
        ],

        'treatment' => [
            'title' => 'Na czym polega zabieg?',
            'paragraphs' => [
                'Podolog ocenia aktualny stan paznokcia, skóry i tkanek okołopaznokciowych.',
                'Następnie dobierana jest odpowiednia metoda zabezpieczenia, korekcji lub odciążenia.',
                'Na zakończenie pacjent otrzymuje zalecenia dotyczące dalszej pielęgnacji oraz ewentualnej kontroli.',
            ],
        ],

        'steps' => [
            [
                'title' => 'Ocena',
                'desc' => 'Analiza paznokcia i tkanek.',
            ],
            [
                'title' => 'Zabieg',
                'desc' => 'Wykonanie korekcji, zabezpieczenia lub odciążenia.',
            ],
            [
                'title' => 'Kontrola',
                'desc' => 'Zalecenia i dalsza opieka.',
            ],
        ],

        'category' => 'wrastajace-paznokcie',

        'tags' => [
            'taping podologiczny',
            'odciążenie',
            'wrastające paznokcie',
        ],

    ],
    'opatrunek-z-odciazeniem' => [
        'slug' => 'opatrunek-z-odciazeniem',

        'seo' => [
            'title' => 'Opatrunek z odciążeniem Kielce | Podolog OAZA',
            'description' => 'Specjalistyczny opatrunek z odciążeniem stosowany w celu ochrony bolesnego miejsca i poprawy komfortu chodzenia. Gabinet podologiczny OAZA Kielce.',
        ],

        'hero' => [
            'title' => 'Opatrunek',
            'titleSecond' => 'z odciążeniem',
            'text' => 'Specjalistyczny opatrunek z odciążeniem stosowany w celu ochrony bolesnego miejsca i poprawy komfortu chodzenia.',
            'price_from' => '100 zł',
        ],

        'symptoms' => [
            'Wrastający lub bolesny paznokieć',
            'Stan zapalny wałów okołopaznokciowych',
            'Ból i ucisk podczas chodzenia',
            'Potrzeba zabezpieczenia miejsca po zabiegu',
            'Kontynuacja terapii podologicznej',
        ],

        'treatment' => [
            'title' => 'Na czym polega zabieg?',
            'paragraphs' => [
                'Podolog ocenia aktualny stan paznokcia, skóry i tkanek okołopaznokciowych.',
                'Następnie dobierana jest odpowiednia metoda zabezpieczenia, korekcji lub odciążenia.',
                'Na zakończenie pacjent otrzymuje zalecenia dotyczące dalszej pielęgnacji oraz ewentualnej kontroli.',
            ],
        ],

        'steps' => [
            [
                'title' => 'Ocena',
                'desc' => 'Analiza paznokcia i tkanek.',
            ],
            [
                'title' => 'Zabieg',
                'desc' => 'Wykonanie korekcji, zabezpieczenia lub odciążenia.',
            ],
            [
                'title' => 'Kontrola',
                'desc' => 'Zalecenia i dalsza opieka.',
            ],
        ],

        'category' => 'wrastajace-paznokcie',

        'tags' => [
            'opatrunek z odciążeniem',
            'odciążenie stopy',
            'ochrona rany',
        ],

    ],
    'opatrunek-z-preparatem-specjalistycznym' => [
        'slug' => 'opatrunek-z-preparatem-specjalistycznym',

        'seo' => [
            'title' => 'Opatrunek z preparatem Kielce | Podolog OAZA',
            'description' => 'Specjalistyczny opatrunek z preparatem dobranym do problemu, wspierający regenerację i ochronę tkanek. Gabinet podologiczny OAZA Kielce.',
        ],

        'hero' => [
            'title' => 'Opatrunek',
            'titleSecond' => 'z preparatem',
            'text' => 'Specjalistyczny opatrunek z preparatem dobranym do problemu, wspierający regenerację i ochronę tkanek.',
            'price_from' => '100 zł',
        ],

        'symptoms' => [
            'Wrastający lub bolesny paznokieć',
            'Stan zapalny wałów okołopaznokciowych',
            'Ból i ucisk podczas chodzenia',
            'Potrzeba zabezpieczenia miejsca po zabiegu',
            'Kontynuacja terapii podologicznej',
        ],

        'treatment' => [
            'title' => 'Na czym polega zabieg?',
            'paragraphs' => [
                'Podolog ocenia aktualny stan paznokcia, skóry i tkanek okołopaznokciowych.',
                'Następnie dobierana jest odpowiednia metoda zabezpieczenia, korekcji lub odciążenia.',
                'Na zakończenie pacjent otrzymuje zalecenia dotyczące dalszej pielęgnacji oraz ewentualnej kontroli.',
            ],
        ],

        'steps' => [
            [
                'title' => 'Ocena',
                'desc' => 'Analiza paznokcia i tkanek.',
            ],
            [
                'title' => 'Zabieg',
                'desc' => 'Wykonanie korekcji, zabezpieczenia lub odciążenia.',
            ],
            [
                'title' => 'Kontrola',
                'desc' => 'Zalecenia i dalsza opieka.',
            ],
        ],

        'category' => 'wrastajace-paznokcie',

        'tags' => [
            'opatrunek podologiczny',
            'preparat specjalistyczny',
            'regeneracja',
        ],

    ],
];
